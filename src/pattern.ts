...
export class AccessPattern {
  include: Set<string>;
  exclude: Set<string>;
  private includeTester: TokenTester;
  private excludeTester: TokenTester;
  ...
  toWhere(key: string): unknown {
    const includeArray = Array.from<string, unknown>(this.include, (token) => {
      if (token.endsWith("*")) {
        return {
          [key]: {
            _like: token.replace("*", "%")
    ...
    const excludeArray = Array.from<string, unknown>(this.exclude, (token) => {
      if (token.endsWith("*")) {
        return {
          [key]: {
